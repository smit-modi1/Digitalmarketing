...
const blobProductsPath = "dailydealsuk/products.json";
const blobMessagesPath = "dailydealsuk/messages.json";
const blobArticlesPath = "dailydealsuk/articles.json";
const blobAdminUsersPath = "dailydealsuk/admin-users.json";
const blobSubscribersPath = "dailydealsuk/subscribers.json";
const blobNewsletterLogsPath = "dailydealsuk/newsletter-logs.json";
...
async function getArticles() {
  const storedArticles = await readDocument(blobArticlesPath, fallbackArticles);
  const mergedArticles = mergeSeededArticles(storedArticles, fallbackArticles);

  if (mergedArticles.length !== storedArticles.length) {
    await writeDocument(blobArticlesPath, mergedArticles);
  }
...
function mergeSeededArticles(storedArticles, seededArticles) {
  ...
  const merged = Array.isArray(storedArticles) ? [...storedArticles] : [];
  seededArticles.forEach((article) => {
    if (!existingSlugs.has(article.slug)) {
      merged.push(cloneValue(article));
    }
  });
